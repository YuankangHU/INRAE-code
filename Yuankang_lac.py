...
def process_satellite_data(oauth, evalscript, start_date, end_date, new_bbox, utm_crs, pixel, id, client_id, client_secret):
    ...
    for single_date in daterange(start_date, end_date):
        ...
        folder_name = os.path.join('data', f"{id}_{year}_result", formatted_date)
...
def check_and_delete_empty_directories(start_date, end_date,id):
    for single_date in daterange(start_date, end_date):
        ...
        directory_path = os.path.join(current_working_directory, 'data', f"{id}_{single_date.year}_result", formatted_date)    
...
def split_bands(single_date, id):
    ...
    date_folder = os.path.join('data',f"{id}_{year}_result", single_date)
...
def process_chla_map(single_date,id):
    ...
    if cloud_coverage_percentage <= 60 and snow_coverage_percentage == 0:
        ...
        current_working_directory = os.getcwd()
        new_directory = os.path.join(current_working_directory, 'data',f"{id}_{year}_movie")
        ...
        if coordinates:
            ...
            if polygon:
                ...
                for year in range(start_year, end_year + 1):
                    directory_path = os.path.join('data', f"{id}_{year}_result")
